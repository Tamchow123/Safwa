 ...
 *
 * The detail link and the bookmark toggle are DOM SIBLINGS (Phase 14 §13):
 * a `<button>` nested inside an `<a>` is invalid HTML and produces
 * unreliable keyboard/touch behaviour, so the bookmark control sits
 * alongside the link, not inside it. `data-entry-id`/`data-bab`/
 * `data-verb-type`/`data-book-page` stay on the link itself, matching
 * existing test expectations that query them via the link's role.
 */
  ...
  return (
    <div className="relative">
      <Link
        ...
        className="bg-card hover:border-primary/40 focus-visible:border-primary block min-h-11 rounded-xl border p-4 pr-14 transition-colors"
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
            <ArabicText className="text-2xl font-medium">
              {entry.madi}
            </ArabicText>
